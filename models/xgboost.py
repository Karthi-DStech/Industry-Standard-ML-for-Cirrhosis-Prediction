...
class XgBoostModel(BaseModel):
    ...
    def __init__(self, logger, opt, missing_value=None):
        """
        Initialize the XgBoostModel object with the logger and options.
        ...
        missing_value : str
            The missing value strategy to be used.
        """
        super().__init__(logger, opt)
        self._model_name = "XGBClassifier"
        self.missing_value = self._opt.missing_value_setup_model

    def get_model(self, **kwargs):
        """
        This method returns a XGBoost instance.
        ...
        **kwargs : dict
        ...
        if self._opt.missing_value_model == True:
            self.logger.update_log(
                "model_training", "Model was opted to handle missing values", "True"
            )
            self.logger.update_log(
                "model_training", "Model handles missing value as ", self.missing_value
            )
            kwargs["missing"] = self.missing_value

        else:
            self.logger.update_log(
                "model_training", "Model was opted to handle missing values", "False"
            )
